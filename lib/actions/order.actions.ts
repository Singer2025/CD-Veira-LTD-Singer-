...
export const calcDeliveryDateAndPrice = async ({
  ...
  // Check if any items are large appliances (eligible for shipping)
  const hasLargeAppliances = items.some(item => item.isLargeAppliance)
  
  // Only calculate shipping price if there are large appliances
  // For small appliances, shipping is not available (in-store pickup only)
  let shippingPrice = 0
  if (hasLargeAppliances && shippingAddress) {
    // If there are large appliances, shipping is free for large appliances
    shippingPrice = 0
  } else {
    // If there are no large appliances or no shipping address, no shipping cost
    // Small appliances are in-store pickup only
    shippingPrice = 0
  }
  ...
  return {
    ...
    taxPrice,
    totalPrice,
    hasLargeAppliances, // Add this flag to indicate if the order has large appliances
  }
}
