...
export function formUrlQuery({
  ...
  // Use the provided path or default to current path
...
export const toSlug = (text: string | number | boolean): string =>
  String(text)
    .toLowerCase()
    .replace(/[^\w\s-]+/g, '')
    .replace(/\s+/g, '-')
...
export const getFilterUrl = ({
  ...
  page,
  brand,
  locale = 'en-US', // Add locale parameter with default value
}: {
  params: {
    q?: string
    category?: string
    ...
    price?: string
    rating?: string
    brand?: string // Add brand to params type
    sort?: string
    page?: string
  }
  tag?: string
  ...
  rating?: string
  page?: string
  brand?: string // Add brand as an individual parameter
  locale?: string // Add locale parameter type
}) => {
  ...
  if (brand) newParams.brand = brand // Add brand to newParams
  return `/${locale}/search?${new URLSearchParams(newParams).toString()}`
}
