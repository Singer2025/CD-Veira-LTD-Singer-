...
const Price = (field: string) =>
  z.coerce
    .number()
    .min(0.01, `${field} must be greater than 0`)
    .refine(
...
// Category Schemas
// Base object schema for Category
...
// Apply refinements for the Input schema
export const CategoryInputSchema = CategoryObjectSchema
  .refine(data => data.isParent || data.parent, {
    message: "Parent category is required when not a parent category",
    path: ["parent"]
  })
  .refine(data => !data.isParent || (data.isParent && data.image), {
    message: "Image is required for parent categories",
    path: ["image"]
  });

// Extend the base object schema and apply the same refinements for the Update schema
export const CategoryUpdateSchema = CategoryObjectSchema.extend({
  _id: MongoId
}).refine(data => data.isParent || data.parent, {
  message: "Parent category is required when not a parent category",
  path: ["parent"]
...
export const ProductInputSchema = z.object({
  ...
  category: z.union([MongoId, z.string().min(1, 'Category is required')]), // Allow string or MongoId
  images: z.array(z.string()).min(1, 'Product must have at least one image'),
  brand: z.union([MongoId, z.string().min(1, 'Brand is required')]), // Allow string or MongoId
  description: z.string().min(1, 'Description is required'),
  isPublished: z.boolean(),
  listPrice: Price('List price'),
  // Sale price - completely optional field (discounted price)
  price: z.preprocess(
    (val) => {
    ...
    z.union([
      z.coerce
        ...
        .refine(
          (value) => /^\d+(\.\d{2})?$/.test(formatNumberWithDecimal(value)),
          'Sale price must have exactly two decimal places (e.g., 49.99)'
        ),
      z.undefined()
    ])
  ).optional(),
  ...
  // Appliance/Furniture specific fields
  dimensions: z.object({
  ...
  isShippable: z.boolean().optional().default(true),
  isPickupAvailable: z.boolean().optional().default(false),
...
export const OrderItemSchema = z.object({
  ...
  isLargeAppliance: z.boolean().optional(),
})
export const ShippingAddressSchema = z.object({
  fullName: z.string().min(1, 'Full name is required'),
  street: z.string().min(1, 'Address is required'),
  city: z.string().min(1, 'City is required'),
  postalCode: z.string().optional(), // Optional for Saint Vincent and the Grenadines
  province: z.string().min(1, 'Parish is required'),
...
export const CartSchema = z.object({
  ...
  fulfillmentMethod: z.optional(z.enum(['shipping', 'pickup', 'special'])).default('shipping'),
  pickupLocation: z.optional(z.string()),
